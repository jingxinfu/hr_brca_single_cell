...
readmtrx <-function(file){
    d <- fread(file, stringsAsFactors = F, data.table = F)
    # remove duplicated genes and genes without expression
    d <- d[!duplicated(d[,1]),]
    d <- data.frame(d[,-1,drop=FALSE], row.names = d[,1], check.names = F)
    d <- d[rowSums(d)!=0,]
    return(d)
}
